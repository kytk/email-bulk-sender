...
class EmailBulkSenderGUI(ctk.CTk):
    def __init__(self):
        ...
        # 設定マネージャーの初期化
        self.config_manager = ConfigManager("email")
    ...
    def load_config_settings(self):
        ...
        if smtp_config.get('server'):
            self.smtp_server.set(smtp_config['server'])
        if smtp_config.get('port'):
            self.smtp_port.set(str(smtp_config['port']))

        # 送信者情報
        sender_config = config.get('sender', {})
        if sender_config.get('email_address'):
            self.email_address.set(sender_config['email_address'])
    ...
    def save_config_settings(self):
        ...
        config_to_save = {
            ...
            "smtp": {
                ...
                "port": int(self.smtp_port.get()) if self.smtp_port.get() else 587
            },
            "sender": {
