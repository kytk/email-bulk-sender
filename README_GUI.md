...
配布ZIPファイルには以下が含まれています：

| ファイル | 説明 |
|---------|------|
| `EmailBulkSender_Setup.exe` | インストーラー |
| `examples/` | 受信者リストとテンプレートのサンプル |
| `README.html` | このファイル |

**インストール手順：**

1. `EmailBulkSender_Setup.exe` を実行します
2. 画面の指示に従ってインストールを進めます
3. スタートメニューまたはデスクトップのショートカットから起動できます

**「WindowsによってPCが保護されました」と表示された場合：**

このツールにはコード署名がないため、Windows SmartScreen の警告が表示されることがあります。以下の手順で実行できます。
...
`examples/` フォルダには受信者リストとテンプレートのサンプルが入っています。デスクトップなどの作業しやすい場所にコピーしてご利用ください。
...
| ファイル | 説明 |
|---------|------|
| `EmailBulkSender_mac_*.dmg` | インストーラー（DMG） |
| `examples/` | 受信者リストとテンプレートのサンプル |
| `README.html` | このファイル |
...
2. 表示されたウィンドウで、アプリアイコンを「Applications」フォルダにドラッグ＆ドロップします
3. Launchpad または Applications フォルダから「メール一括送信ツール」を起動します
...
- **設定を保存** - 現在の設定をファイルに保存します。パスワードはOSの安全な保存領域（Windows: Credential Store / macOS: Keychain）に保存されます
